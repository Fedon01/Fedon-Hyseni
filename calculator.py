...
class Calculator:
    def __init__(self,master):
        ...
        self.equation = StringVar()  #hold an empty string
        self.entry_value=""
        Entry(width=17,bg="#fff",font=("Arial Bold",28),textvariable=self.equation).place(x=0,y=0)

        Button(width=11, height=4, text="(", relief="flat", bg="white", command=lambda: self.show("(")).place(x=0, y=50)  #add value to show
    ...
    def show(self,value):
        self.entry_value+=str(value)  #make a string every value that comes
        self.equation.set(self.entry_value) #append value of this box

    def clear(self):
        self.entry_value=""
        self.equation.set(self.entry_value) #set empty string

    def solve(self):
        result=eval(self.entry_value)  #build in function to math calculations
        self.equation.set(result)  #append to frame
